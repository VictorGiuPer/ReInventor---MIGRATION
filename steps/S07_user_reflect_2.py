...
def context_summary_2(context_summary: str, critique_text: str, user_feedback: str) -> str:
    """
    Calls the LLM with context, critique, and user feedback,
    and returns the updated context summary.
    """

    # Build a prompt that acknowledges the user's Round 2 reflections
    # and updates the working context for the next critique stage
    prompt = f"""
        You are the Context Updater in the Idea Hardener process.

    We have completed Round 2 of critique. Your job is to acknowledge the user's reflections and produce an updated context summary that will carry forward into the next step.
    ...
    Here is the **critique output** from Round 2:
    ---
    {critique_text}
    ---
    ...
    **Part 2 — Updated Context Summary**
    ...
    - Key reflections and position updates from ALL critique rounds so far (not just Round 2)
    ...
    # Single LLM call to acknowledge feedback and consolidate context
    ...
    # Return raw model output for downstream parsing
